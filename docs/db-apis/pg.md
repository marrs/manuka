> [!NOTE]
> This document was mostly generated by AI.  If you're investigating a bug,
> consider the possibility that the code could have been written under false
> assumptions.

# PostgreSQL Client Protocol Documentation

This document explains what **PostgreSQL database** expects from client
applications when executing prepared statements and binding parameters. We use
the `pg` npm library as a reference implementation to show how these
requirements are met in practice.

This document is intended to help make decisions about how Manuka should go
about formatting statements and preparing data.

**Key Distinction:**
- **PostgreSQL Requirement** = What the database protocol mandates
- **`pg` Implementation** = How the library implements those requirements
